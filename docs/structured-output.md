...
| GPT-6 Astra (**Bedrock** `bedrock-mantle` OpenAI-compat) | adaptive (server-side) | `false` | prompt | Two reasons beyond the assume-prompt-parsing default: same adaptive server-side reasoning as the GPT-5.x entries at this same endpoint (where the reasoning-only failure was live-verified on 5.5), and its Bedrock model card lists *Structured outputs: Not Supported* on `bedrock-runtime`. **Not** live-probed — flipping it to `true` needs a run over think-heavy batches, not a doc. Both Responses and Chat Completions work here (unlike Sol), but we keep Responses for reasoning summaries; no `temperature`/`top_p`. us-west-2 only |
...
Fable 5 on Bedrock, GPT-5.6 Sol and GPT-6 Astra on `bedrock-mantle`, GLM 5 on Bedrock, and the GLM-5.3 family
