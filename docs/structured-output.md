...
thinking model, discussed above. Losing GLM 5 left **Bedrock serving Claude only** until Kimi K3 (Bedrock) arrived 2026-09-23, so
the "third-party Bedrock re-host" case has an instance again — live-verified rather than assumed
(see its row in the matrix). In exchange, the K3 row is the first
