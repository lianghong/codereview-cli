...
- **Classify retryability on the HTTP *status*, not on an exception class** — an `isinstance` against a type the *installed* client never raises is dead code, and dead retry logic is invisible: a misclassified throttle looks exactly like a lost batch. Two classifiers were dead this way and aborted real 429/503/504s on attempt 1. Provider status sets differ **on purpose** — recorded in `_RETRY_MATRIX`, not normalized (`tests/test_retry_contract.py`). **This is why `langchain_core.exceptions`' standard `ModelError`/`is_retryable` hierarchy was rejected** (researched 2026-09-19): only `langchain-openai` and `langchain-google-genai` raise those types, so an `is_retryable` branch is dead on Bedrock and NVIDIA. They're mixins over the vendor's own exception, so they break nothing — just don't classify on them. → `docs/providers.md`
- **Our retry loop must be the only one.** Every client is constructed with its own retries off — `max_retries=CLIENT_RETRIES_DISABLED` (`mixins.py`) for the six OpenAI-SDK-shaped clients, `retries={"max_attempts": 0}` in Bedrock's `BotocoreConfig`; NVIDIA's `requests.Session` has no retry adapter and needs nothing. **Nested loops multiply, they don't add**: the parameter was merely *absent*, so the openai SDK's `DEFAULT_MAX_RETRIES = 2` turned a budget of 5 into 6×3 = **18** requests and `ChatGoogleGenerativeAI`'s declared 6 into 6×7 = **42**. Worse than the waste, the inner loop retries on *its* policy — `_is_retryable_error`/`_RETRY_MATRIX` never see the failure — and its sub-second backoff pre-empts Azure's `Retry-After` and Google's 10s 429 base. **Never set a nonzero value as a "safety net"**; that net is what hid the doubling. Two guards in `tests/test_retry_contract.py`, one asserting the constructor kwargs, one reflecting over the provider package. → `docs/providers.md`
...
**Configurable via `models.yaml` (no code changes):** model registration, `aliases` / `deprecated_aliases`, pricing, inference params, `context_window`, `supports_tool_use`, `use_responses_api`, AWS region (provider-level plus a per-model `region` override for region-restricted models — on `bedrock` it selects the Converse client's region, on `bedrock_openai` it rewrites the Region label of `base_url`), Bedrock `read_timeout`, NVIDIA's provider-level `max_retries` and `polling_timeout`, scanning patterns/extensions, max file size. Two per-model overrides carry reasons worth knowing: fable5 pins `us-east-1` because its `provider_data_share` opt-in is per-region account state, and **any new Bedrock model whose thinking is on without being asked for needs `read_timeout: 1800`** (the non-streaming Converse path emits no bytes until the full response is generated, so think-heavy batches hit `ReadTimeoutError` against the 300s default). **NVIDIA has the same trap with a worse default:** `polling_timeout` is forwarded to `ChatNVIDIA` as `timeout`, and `_NVIDIAClient.timeout` is the `session.post` **read** timeout as well as the 202-poll budget its docstring describes — at the package's default of **60s** no always-reasoning NIM model can finish a batch, and `ReadTimeout` is retryable so it burns the whole budget failing identically. Keep it wired (`test_nvidia_client_timeout_is_wired_and_does_not_reach_the_request_body`); it was dead config once because the kwarg used to land in the request body and draw an HTTP 400, which is no longer true.
...
- **`OPENAI_API_KEY` for OpenAI-on-Bedrock is an Amazon Bedrock API key** (bearer token), *not* an openai.com key, and `OPENAI_BASE_URL` must be a region that actually serves the model — or the model entry must declare `region:`, which overrides the URL's Region label.
...
- **Reasoning models don't accept `temperature`/`top_p`** (Claude Opus 5 / Sonnet 5 / Fable 5, GPT-5.4 / 5.4 Pro on Azure, GPT-5.6 Sol and GPT-6 Astra on Bedrock, DeepSeek-V4-Pro) — omit `default_temperature`; Bedrock and Azure pass `allow_none=True` to `_resolve_temperature`. **Gemini sampling params are deprecated from 3.6 Flash onward** — omit all three for every new Gemini entry (3.1 Pro keeps theirs); `test_gemini38_flash_matches_the_published_model_card` pins the only current Flash and `test_every_modern_gemini_entry_omits_sampling_params` catches the next entry.
- **13 of 19 entries ship `supports_tool_use: false`** — the Claude-on-Bedrock reasoning tiers (Opus 5, Sonnet 5, Fable 5), Kimi K3 on Bedrock, `bedrock-mantle`'s GPT-5.6 Sol and GPT-6 Astra, Kimi K3 and both GLM-5.3 re-hosts on NVIDIA, the Kimi K3 direct API (Moonshot), both current GLM direct entries (Z.AI), and Gemini 3.8 Flash. **Every NVIDIA NIM entry is now on the prompt path** — all three are always-reasoning models with no off switch, and the one NIM entry that sat on the tool-use path did so only because `thinking: false` disabled its reasoning (DeepSeek-V4-Flash, removed 2026-09-19). Only **Opus 5** has vendor confirmation that structured output is unsupported, and **GPT-6 Astra** is live-verified by A/B (forced `tool_choice` on a think-heavy batch → Responses-API `server_error` and a half-finished review; the prompt path finished the same target in 37s — note a *trivial* batch passes cleanly, so small probes prove nothing); most of the rest are the assume-prompt-parsing rule, not a live failure. **Kimi K3 (Moonshot)** is the cheapest live confirmation — one forced `tool_choice` returns HTTP 400 *"incompatible with thinking enabled"*, no think-heavy batch needed. **Kimi K3 (Bedrock)** is the opposite trap — a trivial forced `tool_choice` returns a real `toolUse`, yet a real review failed every attempt with `list_type` (A/B 2026-09-23). **No entry currently holds a live-proven `true` on a thinking model**: Gemini 3.7 Flash was the only one and its entry was curated away 2026-09-19, so the bar for flipping 3.8 or either GLM-5.3 entry is now recorded only in `docs/structured-output.md` — reproduce three clean runs with `parsing_error` None and `output_token_details.reasoning > 0`. An **Azure Foundry deployment of an open-weight model** needs the same flag — SGLang/vLLM reject a forced `tool_choice` unless started with `--enable-auto-tool-choice` — and `tests/test_azure_provider.py::test_supports_tool_use_false_uses_prompt_parsing` keeps that shape as the reference case. Per-entry matrix → `docs/structured-output.md`
...
- **OpenAI-on-Bedrock is NOT the `bedrock` provider.** GPT-5.6 Sol uses Bedrock's *OpenAI-compatible* `bedrock-mantle` endpoint — `ChatOpenAI` + `base_url` with a Bedrock **API key (bearer token)**, not SigV4 `ChatBedrockConverse` — in the separate `bedrock_openai` provider. Its `full_id` must be a **literal**: an unset `${BEDROCK_OPENAI_MODEL_ID}` expands to `""`, fails `min_length=1` and breaks `--list-models`. GPT-5.4 on *Azure* is a separate entry keeping `supports_tool_use: true`. **The two entries' Regions don't overlap** and `base_url` is a *provider*-level setting: GPT-5.6 Sol is **In-Region us-east-1/us-east-2 only**, GPT-6 Astra is **us-west-2 only** on `bedrock-mantle`, and the wrong Region 404s the model id on every batch with no fallback. Both entries therefore carry **`region:`**, which `_resolve_base_url` reads to rewrite the Region label of the configured URL per model, so one export serves both. Three rules there: **derive from the configured URL, never a hardcoded host template** (`OPENAI_BASE_URL` stays authoritative for scheme/host/path, and a custom gateway keeps working); **a host with no Region label passes through unchanged**, debug-logged — failing closed breaks a working self-hosted endpoint over a cosmetic mismatch; and **`require_https` runs on the *resolved* URL**, so an override can't put the bearer key on `http://`. `--validate` can't catch a wrong Region — it has no model in scope. **GPT-6 Astra is the first entry with *tiered* pricing** ($11/$55 per M up to 272K input, $22/$82.50 above). Its `context_window` was clamped to 272000 so the batcher couldn't pack past the break; the clamp is gone now that `PricingConfig` carries `long_context_threshold_tokens` + `long_input_per_million` + `long_output_per_million` (all three or none — a partial tier is a `ValueError`). **The threshold is per *request*, never per run**: five 100K batches total 500K tokens and still bill entirely at the cheap rate, so tier selection happens in `TokenTrackingMixin._track_tokens`, where one call's size is still visible, and `estimate_cost` only *reports* what accrued — it must not recompute from the totals. `--dry-run` does the same per batch in `_estimate_tiered_cost`. **Every cost consumer reads that accrual off `estimate_cost()` / `ReviewMetrics.input_cost`+`output_cost`, never `tokens × rate`** — the run summary and the Markdown export each recomputed from the *totals* at the short rate and reported half the billed figure; `test_no_new_consumer_recomputes_cost_from_a_rate` allowlists the only three legitimate sites in `_ALLOWED_COST_ARITHMETIC`. → `docs/providers.md`
