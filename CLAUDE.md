...
- **Classify retryability on the HTTP *status*, not on an exception class** — an `isinstance` against a type the *installed* client never raises is dead code, and dead retry logic is invisible: a misclassified throttle looks exactly like a lost batch (two classifiers aborted real 429/503/504s on attempt 1 this way). Provider status sets differ **on purpose** — recorded in `_RETRY_MATRIX`, not normalized (`tests/test_retry_contract.py`). **Don't classify on `langchain_core.exceptions`' `ModelError`/`is_retryable`** — only `langchain-openai` and `langchain-google-genai` raise them, so the branch is dead on Bedrock and NVIDIA (rejected 2026-09-19). → `docs/providers.md`
- **Our retry loop must be the only one.** Every client is constructed with its own retries off — `max_retries=CLIENT_RETRIES_DISABLED` (`mixins.py`) for the six OpenAI-SDK-shaped clients, `retries={"max_attempts": 0}` in Bedrock's `BotocoreConfig`; NVIDIA's `requests.Session` needs nothing. **Nested loops multiply, they don't add** (a budget of 5 became 18 requests on the openai SDK, 42 on Google), and the inner loop retries on *its* policy — `_RETRY_MATRIX` never sees the failure and its sub-second backoff pre-empts `Retry-After`. **Never set a nonzero value as a "safety net"**; that net is what hid the multiplication. Two guards in `tests/test_retry_contract.py`. → `docs/providers.md`
...
**Configurable via `models.yaml` (no code changes):** model registration, `aliases` / `deprecated_aliases`, pricing, inference params, `context_window`, `supports_tool_use`, `use_responses_api`, AWS region (provider-level plus a per-model `region` — on `bedrock` the Converse client's region, on `bedrock_openai` the Region label of `base_url`), Bedrock `read_timeout`, NVIDIA's provider-level `max_retries` and `polling_timeout`, scanning patterns/extensions, max file size. **Any new always-thinking Bedrock model needs `read_timeout: 1800`** — non-streaming Converse sends no bytes until generation finishes, so think-heavy batches `ReadTimeoutError` against the 300s default. **NVIDIA has the same trap with a worse default:** `polling_timeout` becomes `ChatNVIDIA(timeout=)`, which is the `session.post` **read** timeout too; the package's 60s lets no always-reasoning NIM model finish, and `ReadTimeout` is retryable, so it burns the budget failing identically. Keep it wired (`test_nvidia_client_timeout_is_wired_and_does_not_reach_the_request_body`). → `docs/providers.md`
...
per-provider setup walkthroughs. Two things worth knowing here:
