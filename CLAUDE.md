...
- **Reasoning models don't accept `temperature`/`top_p`** (Claude Opus 5 / Sonnet 5 / Fable 5, GPT-5.4 / 5.4 Pro on Azure, GPT-5.6 Sol on Bedrock, DeepSeek-V4-Pro) — omit `default_temperature`; Bedrock and Azure pass `allow_none=True` to `_resolve_temperature`. **Gemini sampling params are deprecated from 3.6 Flash onward** — omit all three for every new Gemini entry (3.1 Pro keeps theirs); `test_gemini37_flash_omits_sampling_params` pins the current Flash and `test_every_modern_gemini_entry_omits_sampling_params` catches the next entry.
- **9 of 18 entries ship `supports_tool_use: false`** — the Claude-on-Bedrock reasoning tiers (Opus 5, Sonnet 5, Fable 5), `bedrock-mantle`'s GPT-5.6 Sol, the GLM re-host on Bedrock, MiniMax M3 and Kimi K3 on NVIDIA, and the Kimi / GLM direct APIs (Moonshot, Z.AI). Only **Opus 5** has vendor confirmation (its model card lists *Structured outputs: Not Supported*); most of the rest are the assume-prompt-parsing rule, not a live failure. **Gemini 3.7 Flash** is the only thinking model that won `true` back with a live run — that's the bar for flipping one. An **Azure Foundry deployment of an open-weight model** needs the same flag — SGLang/vLLM reject a forced `tool_choice` unless started with `--enable-auto-tool-choice` — and `tests/test_azure_provider.py::test_supports_tool_use_false_uses_prompt_parsing` keeps that shape as the reference case. Per-entry matrix → `docs/structured-output.md`
