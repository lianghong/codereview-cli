...
- **Reasoning models don't accept `temperature`/`top_p`** (Claude Opus 5 / Sonnet 5 / Fable 5, GPT-5.4 / 5.4 Pro on Azure, GPT-5.6 Sol and GPT-6 Astra on Bedrock, DeepSeek-V4-Pro) — omit `default_temperature`; Bedrock and Azure pass `allow_none=True` to `_resolve_temperature`. **Gemini sampling params are deprecated from 3.6 Flash onward** — omit all three for every new Gemini entry (3.1 Pro keeps theirs); `test_gemini37_flash_omits_sampling_params` pins the oldest current Flash, `test_gemini38_flash_matches_the_published_model_card` pins the newest, and `test_every_modern_gemini_entry_omits_sampling_params` catches the next entry.
- **12 of 21 entries ship `supports_tool_use: false`** — the Claude-on-Bedrock reasoning tiers (Opus 5, Sonnet 5, Fable 5), `bedrock-mantle`'s GPT-5.6 Sol and GPT-6 Astra, the GLM re-host on Bedrock, MiniMax M3 and Kimi K3 on NVIDIA, the Kimi direct API (Moonshot), both current GLM direct entries (Z.AI), and Gemini 3.8 Flash. Only **Opus 5** has vendor confirmation that structured output is unsupported, and **GPT-6 Astra** is live-verified by A/B (forced `tool_choice` on a think-heavy batch → Responses-API `server_error` and a half-finished review; the prompt path finished the same target in 37s — note a *trivial* batch passes cleanly, so small probes prove nothing); most of the rest are the assume-prompt-parsing rule, not a live failure. **Gemini 3.7 Flash** is the only thinking model that won `true` back with a live run — that's the bar for flipping 3.8 or either GLM-5.3 entry. An **Azure Foundry deployment of an open-weight model** needs the same flag — SGLang/vLLM reject a forced `tool_choice` unless started with `--enable-auto-tool-choice` — and `tests/test_azure_provider.py::test_supports_tool_use_false_uses_prompt_parsing` keeps that shape as the reference case. Per-entry matrix → `docs/structured-output.md`
