...
| GPT-5.6 Sol (**Bedrock** `bedrock-mantle` OpenAI-compat) | adaptive (server-side) | `false` | prompt | Same `bedrock-mantle` endpoint and reasoning-only failure mode as GPT-5.5/5.4-on-Bedrock. **Responses API only** (Chat Completions not supported → `use_responses_api: true` required), no `temperature`/`top_p`. Sol tier = OpenAI's best coding model; In-Region us-east-1/us-east-2 only |
| Grok 4.3 (**Bedrock** `bedrock-mantle` OpenAI-compat) | reasoning-first (always-on, effort configurable) | `false` | prompt | Same `bedrock-mantle` endpoint as GPT-5.5/5.4-on-Bedrock; always-on reasoning is the highest-risk forced-`tool_choice`-while-thinking profile → assume-prompt-parsing until proven. Unlike GPT-5.x here it **accepts** `temperature`/`top_p`, so it uses Chat Completions (no `use_responses_api`) |
...
| GLM-5.2 (NVIDIA) | on by default (effort levels) | `false` | prompt | 753B MoE, 1M context. NIM re-host emits malformed/truncated tool-call JSON (as the deprecated GLM-5.1-on-NVIDIA endpoint did), and it's a thinking model → assume-prompt-parsing rule. **Absorbed the retired `glm51`/`glm5` aliases** (NVIDIA deprecated the free z-ai/glm-5.1 endpoint ~2026-07). Unverified live; flip to `true` only if a live run proves tool-use |
...
Two distinct failure shapes drive the `false` cases: **"can't tool-call at all / mangles the tool call"** (MiniMax family, DeepSeek/Kimi-on-Azure SGLang, Kimi-K2.5-on-Bedrock marker leakage, MiniMax-M2.7/Qwen3.5/GLM-5.2-on-NVIDIA malformed output, GLM-5.1/5.2 fenced JSON, Mistral Small) and **"can tool-call but not *while thinking*"** (Opus 4.7/4.8, Sonnet 5, Fable 5, GLM 5, GPT-5.5/5.4/5.6-Sol-on-Bedrock, Grok 4.3-on-Bedrock, K2.6, Step 3.5/3.7 Flash) — the latter are intermittent, except Fable 5's and other always-on-thinking models (including Grok 4.3, whose reasoning is always on), which are constant. Many NVIDIA-NIM and Bedrock re-host `false` values are set under the **assume-prompt-parsing rule** (thinking model, forced `tool_choice` unproven live), not a confirmed live failure — flip to `true` only if a live run proves tool-use. The Gotchas section has the full per-model detail.
